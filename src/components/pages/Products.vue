<template>
    <div>
        ...
        <div class="text-right mt-4">
            <button class="btn btn-primary" @click=openModal(true) data-target="#productModal">建立新的產品</button>
        </div>
...
export default {
    ...
    methods: {
        getProducts(page = 1){ // (page = 1)是ES6的寫法，只要是沒有傳入page，就會自動帶入1。
        ...
        openModal(isNew, item){
            ...
            if(isNew){
                ...
                this.tempProduct = Object.assign({},item); //Object.assign是ES6的寫法，主要是避免tempProduct直接變成item的值
                this.isNew = false;
            }
        },
        updateProduct(){
        ...
        uploadFile(){
            ...
            this.$http.post(url, formData, {
                ...
                if (response.data.success){
                    // vm.tempProduct.imgUrl = response.data.imageUrl; // 如果只用這一行，response.data不會有getter, setter
